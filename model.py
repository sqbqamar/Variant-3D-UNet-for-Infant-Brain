...
from network import Network, convert_label, label_transform
...
#		labels=label_transform(labels, nlabels)
		predictions = {
			'classes': tf.argmax(logits, axis=-1),
			'probabilities': tf.nn.softmax(logits, name='softmax_tensor')
		}
#		logitss = logits[:,:,:]
#		print(logitss.shape, labels.shape, features.shape)
   #     (?, 32, 32, 32, 4) (?, 32, 32, 32) (?, 32, 32, 32, 2)
...
#		loss = loss + self.conf.weight_decay * tf.add_n(
#				[tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'kernel' in v.name])
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			optimizer = tf.train.AdamOptimizer(learning_rate=self.conf.learning_rate)#, momentum=self.conf.momentum)
